<?php
   session_start();
	 ...
	 $all=$_POST['search_category'];
	 $title=$_POST['search'];
	 $city=$_POST['search_location'];
	 $res=null;
	 if($all=="all"){
		 $statement = "books_ad_table b"; 
         $res=mysqli_query($con,"select b.ad_id,b.photo,b.author,b.title,b.des from {$statement}  LIMIT {$startpoint} , {$limit}");
	 }
	 if($all!="all" && $title=="" && $city==""){
		
		 $statement = "books_ad_table b inner join  book_cats c on b.cat_id=c.cat_id where c.cat_name='$all'"; 
         $res=mysqli_query($con,"select b.ad_id,b.photo,b.author,b.title,b.des from {$statement}  LIMIT {$startpoint} , {$limit}");
	 }
	if($all=="all" && $title!="" && $city==""){
		$statement = "books_ad_table b inner join members_table m on b.user_id=m.user_id where b.title='$title'"; 
        $res=mysqli_query($con,"select b.ad_id,b.photo,b.author,b.title,b.des from {$statement}  LIMIT {$startpoint} , {$limit}");
	}
	if($all!="all" && $title!="" && $city==""){
		
		 $statement = "books_ad_table b inner join  book_cats c on b.cat_id=c.cat_id where c.cat_name='$all' and b.title='$title'"; 
         $res=mysqli_query($con,"select b.ad_id,b.photo,b.author,b.title,b.des from {$statement}  LIMIT {$startpoint} , {$limit}");
	 }
	 if($all=="all" && $title=="" && $city!=""){
		$statement = "books_ad_table b inner join members_table m on b.user_id=m.user_id where m.city='$city'"; 
        $res=mysqli_query($con,"select b.ad_id,b.photo,b.author,b.title,b.des from {$statement}  LIMIT {$startpoint} , {$limit}");
	}
	if($all!="all" && $title=="" && $city!=""){
		$statement = "books_ad_table b inner join members_table m on b.user_id=m.user_id inner join book_cats c on b.cat_id=c.cat_id where m.city='$city' and c.cat_name='$all'"; 
        $res=mysqli_query($con,"select b.ad_id,b.photo,b.author,b.title,b.des from {$statement}  LIMIT {$startpoint} , {$limit}");
	}
	if($all=="all" && $title!="" && $city!=""){
		
		$statement = "books_ad_table b inner join members_table m on b.user_id=m.user_id where b.title='$title' and m.city='$city'"; 
        $res=mysqli_query($con,"select b.ad_id,b.photo,b.author,b.title,b.des from {$statement}  LIMIT {$startpoint} , {$limit}");
	}
	if($all!="all" && $title!="" && $city!=""){
	 ...
	 //$statement = "books_ad_table b inner join members_table m on b.user_id=m.user_id where b.title='$title' or m.city='$city'"; 
